...
while True:
    ...
    if event == "✇ Settings":
        ...
        windows_set = sg.Window("Settings", layout_set, return_keyboard_events=True, element_justification="left", modal=True, font=("Helvetica",15), icon=icon)
        while True:
            event, values = windows_set.read()
            if event == sg.WIN_CLOSED:
                break
